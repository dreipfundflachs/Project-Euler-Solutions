...
# is a quadratic equation in b. There will be integer solutions to this if and
# only if n is such that (1 + 2 * n**2 -2 * n) = (n - 1)**2 + n**2
# is a perfect square. This reduces the problem to finding natural numbers n
# such that (n - 1, n) will yield a primitive Pythagorean triple. We know that
# any such triple is given by Euclid's formula:
...
# which can easily be solved. For p to be an integer, we must have that either
# 2 * q**2 + 1 or 2 * q**2 - 1 is a perfect square, respectively. Once we find
# the first pair (p, q) satisfying these conditions for which n = max(2 * p *
# q, p**2 - q**2) is larger than the given bound, we are done and b is given by
# the solution of the original quadratic equation.
